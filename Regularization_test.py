...
def regularization_experiment(reg, X, reg_mode, n_train=5, components=3):
    ...
    (W, H), loss = train_n_times(n_train, MVR_NMF, X, components, lr=25, regularization=reg, normalization=reg_mode)
...
with open(f"Results/Regularization/L1_reg_{x}", "wb") as f:
    pickle.dump((reg1, exp_var_1, w1, h1), f)

# Save regs2 and exp_var_2 to "regularization_L2" file
with open(f"Results/Regularization/L2_reg_{x}", "wb") as fb:
